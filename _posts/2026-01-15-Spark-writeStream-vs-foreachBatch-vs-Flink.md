...
## Approach 1: Multiple writeStream Queries (The Naïve Pattern)
...
flowchart LR
    subgraph Cluster["Spark Cluster — 3x Resource Consumption"]
        direction TB
        subgraph Q1["Query 1"]
            K1["📥 Kafka Read #1"] --> P1["Parse JSON #1"] --> F1["Flatten #1"] --> S1["🗂️ request_logs"]
        end
        subgraph Q2["Query 2"]
            K2["📥 Kafka Read #2"] --> P2["Parse JSON #2"] --> F2["Flatten #2"] --> S2["🗂️ error_events"]
        end
        subgraph Q3["Query 3"]
            K3["📥 Kafka Read #3"] --> P3["Parse JSON #3"] --> F3["Flatten #3"] --> S3["🗂️ perf_metrics"]
        end
    end
</div>
...
flowchart TD
    ...
    Cache --> W1["🗂️ Write request_logs"]
    Cache --> W2["🗂️ Write error_events"]
    Cache --> W3["🗂️ Write perf_metrics"]
...
flowchart LR
    ...
    R --> |"request fields"| S1["🗂️ Iceberg: request_logs"]
    R --> |"error fields"| S2["🗂️ Iceberg: error_events"]
    R --> |"perf fields"| S3["🗂️ Iceberg: performance_metrics"]
</div>
...
flowchart TD
    ...
    FB --> Parse["Parse JSON / Avro\n(ONCE)"]
    Parse --> Enrich["Flatten, Validate, Enrich\n(ONCE)"]
    Enrich --> Persist["💾 .persist()"]

    Persist --> W1["🗂️ Delta Lake: request_logs\n(.write.mode append)"]
    Persist --> W2["🗂️ Delta Lake: error_events\n(.write.mode append)"]
    Persist --> W3["🗂️ Delta Lake: performance_metrics\n(.write.mode append)"]

    W3 --> UP["unpersist()"]

    style FB fill:#2d6,stroke:#000,color:#fff
    style Persist fill:#26d,stroke:#000,color:#fff
</div>
...
flowchart TD
    ...
    Q1 -->|"Yes"| Q2{"Latency\nrequirement?"}
    Q1 -->|"No / Greenfield"| Flink["✅ Apache Flink\nNative fan-out\nMs latency\nGlobal exactly-once"]

    Q2 -->|"Seconds OK"| FBatch["✅ Spark foreachBatch\nSingle read\nSequential/parallel writes\nPer-sink idempotence"]
    Q2 -->|"Sub-second needed"| Q3{"Can invest in\nFlink ops?"}

    Q3 -->|"Yes"| Flink
    Q3 -->|"No"| FBatch2["⚠️ Spark foreachBatch\nwith short trigger intervals\nAccept latency tradeoff"]
</div>
